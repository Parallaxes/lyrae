...
) -> Result<(Browser, std::sync::Arc<headless_chrome::Tab>), Box<dyn Error>> {
    println!("Navigating to login page...");
    tab.navigate_to("https://sis.powayusd.com/PXP2_Login_Student.aspx?regenerateSessionId=true")
        .expect("Navigated to Login Page");

    println!("Logging in...");
    tab.wait_for_element("#ctl00_MainContent_username")?
        .click()?;
    tab.type_str(&std::env::var("USERNAME").expect("Failed").to_string())?;

    tab.wait_for_element("#ctl00_MainContent_password")?
        .click()?;
    tab.type_str(&std::env::var("PASSWORD").expect("Failed").to_string())?
        .press_key("Enter")?;
...
pub fn fetch_grades(
    ...
    let element = tab.wait_for_element(&format!(
        "div.row:nth-child({}) > div:nth-child(1) > button:nth-child(1)",
        num
    ))?;
    element.click()?;
    let course = element;
...
pub fn process_course(
    ...
    for i in 1..=100 {
        ...
        let date =
            chrono::NaiveDate::parse_from_str(date_str.trim(), "%m/%d/%Y").unwrap_or_else(|_| {
